...
def classifier_label(model, xyxy, device, person_dir, im0, features):
    # crop
    ...
    label = "no info"
    list = os.listdir(person_dir)
    max_f = 0
    t = 0
    for i, each in enumerate(list):
        for item in features[each]:
            t = cosin_metric(item, _feature)
            if t > max_f:
                max_f = t
                max_n = os.path.splitext(each)[0]
        if (max_f > 0.44):
            label = max_n
    return label
...
def get_featuresdict(person_dir):
    list = os.listdir(person_dir)
    person_dict = {}
    for each in list:
        feature = np.load(os.path.join(person_dir, each))
